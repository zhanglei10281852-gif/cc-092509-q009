...
router = APIRouter(prefix="/api/disclosure-packages", tags=["对外披露包"])


@router.post("", status_code=status.HTTP_201_CREATED)
def create_package(payload: DisclosurePackageCreate, principal: Principal = Depends(current_principal)):
    with transaction(immediate=True) as connection:
        return DisclosurePackageService(connection).create(principal, payload.model_dump())
...
@router.post("/{package_id}/items/{item_id}/review")
def review_item(
    package_id: int,
    item_id: int,
    payload: ReviewMark,
    ...
    with transaction(immediate=True) as connection:
        return DisclosurePackageService(connection).review_item(principal, package_id, item_id, payload.model_dump())


@router.post("/{package_id}/signatures", status_code=status.HTTP_201_CREATED)
def sign_package(package_id: int, payload: SignatureCreate, principal: Principal = Depends(current_principal)):
    with transaction(immediate=True) as connection:
