...
- 披露包编排与审查：按档案及版本生成稳定清单摘要，自动剔除仍在保密期的页码、未确认权属的附件和禁止出口的字段；审查人逐项标记保留或剔除并说明依据，法务与保密两名不同角色签字后方可登记发送；底层版本、密级或权属变更会使未完成发送的包失效并重新进入审查；重复发送请求幂等返回，不产生新的外部披露事件；查询展示被剔除内容与完整责任链。
